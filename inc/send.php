...
/**
* Check for heartbeat pulse
* Send notifications to front end
*/
class woptificationsSendNotifications
{
    
    function __construct()
    {
        add_filter('heartbeat_received', array($this, 'woptifications_heartbeat_received'), 10, 2); 
        add_filter('heartbeat_nopriv_received', array($this, 'woptifications_heartbeat_received'), 10, 2);
    }
                ...
                foreach ( $notifications as $db_notification ) {
                    ...
                    if ( false !== ($notification = get_transient($id))) {
                        ...
                        if($post_type == 'product' && $product_cat_match == 1) {
                            ...
                            if($this->woptifications_check_match($viewed_post_id,$post_id,"product_cat")) {
                        ...
                        // check category match
                            ...
                            if($this->woptifications_check_match($viewed_post_id,$post_id,"category")) {
                                $data['woptificationsPopup'][$id] = $notification;
                            }
...
$woptificationsSendNotifications = new woptificationsSendNotifications();
